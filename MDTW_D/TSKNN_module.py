...
def KNN(results_path, dataset_name, dataset, labels, nb_folds=5, dis='euclidean'):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataset_1)):
        ...
        with open(f'{results_path}/dataset_{dataset_name}_KNN_MDTW-D_fold_{fold+1}.txt', 'w') as f:
            f.write(f'Accuracy: {accuracy}\n')
            f.write(f'F1 Score: {f1}\n')
            f.write(f'Confusion Matrix:\n{confusion}\n\n')
            f.write(f'Classification report:\n{report}\n\n')
        
    with open(f'{results_path}/dataset_{dataset_name}_KNN_MDTW-D.txt', 'w') as f:
