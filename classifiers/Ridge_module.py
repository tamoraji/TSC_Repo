...
def Ridge(results_path, dataset_name, dataset, labels, nb_folds=5):
    ...
    if dataset.shape[2] > 1:
        print("Ridge is not capable of doing classification on MTS. so it will be done on only the first dimension")
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataset_1)):
        ...
        with open(f'{results_path}/dataset_{dataset_name}_Ridge_fold_{fold+1}.txt', 'w') as f:
    ...
    with open(f'{results_path}/dataset_{dataset_name}_Ridge.txt', 'w') as f:
