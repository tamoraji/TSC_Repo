...
def LR(results_path, dataset_name, dataset, labels, nb_folds=5,  C=10):
    ...
    if dataset.shape[2] > 1:
        print("LR is not capable of doing classification on MTS. so it will be done on only the first dimension")
    ...
    classifier = LogisticRegression(C=C, max_iter=1000, n_jobs=10)
